#!/usr/bin/env python3
"""
Regenerate embeddings for photos in a specific album using TwelveLabs
"""
...
def get_photos_in_album(album_name):
    ...
    try:
        with get_flask_safe_connection() as conn:
            ...
            cursor.execute("""
                ...
                AND UPPER(album_name) = UPPER(:album_name)
                ORDER BY id
...
def create_photo_embedding(file_path, media_id, file_name):
    ...
    try:
        ...
        try:
            # Create embedding task using embed.create
...
def main():
    print("=" * 60)
    print("🔄 Album-Specific Embeddings Regeneration Script")
    print("=" * 60)
    
    # Get album name from command line or use default
    album_name = sys.argv[1] if len(sys.argv) > 1 else "Rahul Wedding"
    
    # Get photos in album
    print(f"\n📊 Checking for photos in album: {album_name}...")
    photos = get_photos_in_album(album_name)
    
    if not photos:
        print(f"❌ No photos found in album: {album_name}")
        return
    
    print(f"\n📷 Found {len(photos)} photos in album: {album_name}\n")
